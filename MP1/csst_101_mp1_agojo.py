...
Original file is located at
    https://colab.research.google.com/drive/16f2dmOD85D4MoAZqEPa-pkHMgXC37Cpz
"""
...
def grading_rule(student_name):
    ...
    P = grade >= 75           # Has passing grade
    Q = True                  # If passing grade, then student passes
    result = impl(P, Q)
...
def login_rule(student_name):
    ...
    Q = P                               # Access granted only if password is correct

    result = impl(P, Q)
    outcome = "Access Granted" if result and P else "Access Denied"
...
# ---------- Rule 5: Lunch Pass ---------- #
def pass_rule(student_name):
    print("\n--- Lunch Pass Checker ---")
    idcheck = input("Is the student ID valid? (T/F): ").strip().upper() == "T"
    lpass = input("Does the student have a Lunch Pass? (T/F): ").strip().upper() == "T"

    result = impl(idcheck, lpass)
    outcome = "Granted" if result else "Denied"
    print(f"P = {tf(idcheck)} (ID Verified), Q = {tf(lpass)} (Lunch Pass)")
    print("Result:", outcome)

    log_result(student_name, "Lunch Rule", outcome)
